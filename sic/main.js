// ===================================================================================
// MAIN.JS - The Application Controller (Updated with Filling System)
//
// Responsibilities:
...
// 5. Handling the new filling system with protocols and sweetness control.
// ===================================================================================
...
(function(window, Core, UI) {
    ...
    const AppState = {
        ...
        savedRecipes: [],
        currentFillingPreset: null,
        currentFillingMode: 'preset',
        lastPanInputs: {
            normal: null,
            reverse: null,
            filling: null
        ...
        userPreferences: {
            ...
            defaultBakingTime: 7,  // Updated from 8
            defaultThickness: 3,
    ...
    function handleFillingModeToggle(event) {
        ...
        const mode = button.dataset.mode;
        AppState.currentFillingMode = mode;
    ...
    function handleFillingPresetChange(event) {
        ...
        if (!presetId) {
            ...
            AppState.currentFillingPreset = null;
            return;
        }
        
        AppState.currentFillingPreset = Core.FillingService.PRESETS[presetId];
        if (!AppState.currentFillingPreset) return;
        
        // Render ingredients fields
        UI.renderFillingPresetIngredients(AppState.currentFillingPreset, presetId);
        
        // Render preparation protocol
        ...
        // Show sweetness control
        const sweetnessControl = document.querySelector('.sweetness-control');
        if (sweetnessControl) {
            sweetnessControl.style.display = 'block';
        }
    ...
    function updateSweetnessIndicator() {
        ...
        // Get current recipe from inputs
        const currentRecipe = {};
        ...
        if (Object.keys(currentRecipe).length === 0) {
            currentRecipe = AppState.currentFillingPreset.baseRecipe;
        }
        
        // Calculate sweetness
        const sweetness = Core.FillingService.calculateSweetnessIndex(currentRecipe);
    ...
    function handleCalculateFilling() {
        const mode = AppState.currentFillingMode;
        let baseFilling = {};
        
        if (mode === 'preset') {
            // Get from preset inputs
            if (!AppState.currentFillingPreset) {
            ...
            if (Object.keys(baseFilling).length === 0) {
                baseFilling = AppState.currentFillingPreset.baseRecipe;
            }
            
        } else {
            // Get from custom inputs
            const customMapping = {
        ...
        if (!dim1 || (shape === 'rectangle' && !dim2) || !layerCount) {
            alert('يرجى إدخال جميع معايير الكيكة');
            return;
        }
        ...
        if (area === 0) {
            alert('خطأ في حساب مساحة الصينية');
            return;
        }
        
        // Calculate required weight
        const fillingLayers = layerCount - 1; // عدد طبقات الحشو
        const requiredWeight = area * (fillingThickness / 10) * fillingLayers * 1.1; // 1.1 = density
        
        // Apply sweetness reduction if in preset mode
        let result;
        if (mode === 'preset' && $('#sweetness-adjuster')) {
            ...
            result = {
                recipe: scaledRecipe,
                originalSweetness: Core.FillingService.calculateSweetnessIndex(baseFilling),
                newSweetness: Core.FillingService.calculateSweetnessIndex(scaledRecipe),
                reductionApplied: 0
            };
        }
        
        // Calculate additional metrics
        const waterActivity = Core.FillingService.calculateWaterActivity(result.recipe);
        const stability = Core.FillingService.calculateStability(result.recipe);
        ...
        const finalResult = {
            ...
            presetName: AppState.currentFillingPreset?.name || 'مخصص'
        };
    ...
    function init() {
        ...
        // Dynamic ingredient inputs for filling presets
